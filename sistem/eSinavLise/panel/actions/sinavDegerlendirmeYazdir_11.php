<?php
	...
	if ($_GET["sinav"]){
		...
		//Tablo Üst Bilgileri
		$q = mysql_fetch_object(mysql_query("SELECT dersler.dersAdi, sinavlar.sinavTarihi, sinavlar.sinavSaati, sinifseviyeleri.seviye, dallar.dal, kullanicilar.ad, kullanicilar.soyad FROM sinavlar INNER JOIN dallar ON dallar.id = sinavlar.dal INNER JOIN dersler ON dersler.id = sinavlar.ders INNER JOIN kullanicilar ON kullanicilar.id = sinavlar.ogretmen INNER JOIN sinifseviyeleri ON sinifseviyeleri.id = sinavlar.seviye WHERE sinavlar.id = '$s' "));
