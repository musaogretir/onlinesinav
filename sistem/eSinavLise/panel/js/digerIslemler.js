...
//Dal Ekle Bölüm Seçimi
$(document).delegate("#bolumListe_27","change", function(){
	if ($(this).val()>0) $("#dalAdi_27").parents(".altSayfaFormAlaniBilgiSatiri").slideDown(500); else $("#dalAdi_27").parents(".altSayfaFormAlaniBilgiSatiri").slideUp(500);
});

$(document).delegate("#btnOnayDalEkle_27","click", function(){
	msg = "";
	if ($("#bolumListe_27 option:selected").val()>0){
		if ($.trim($("#dalAdi_27").val())!=""){
			$.ajax({
					type	: 'POST',
					data	: 'a='+$("#bolumListe_27 option:selected").val()+'&b='+$("#dalAdi_27").val(),
					url		: 'actions/dalEkle_27.php',
					cache	: 'false',
					success	: function(a){
						if ($.trim(a)=="Dal eklendi."){
							$.Zebra_Dialog(a, {
								'type':     'information',
								'title':    'Bilgi',
								'buttons':  ['Tamam'],
						...
						}else $.Zebra_Dialog(a, {'type': 'error','title':'Hata'});
					},
					error	:function(x,e){
							if(x.status==0){
								msg='Bağlantı kurulamadı!<br/> İnternet bağlantınızı kontrol edin.';
...
$(document).delegate("#btnOnaySinifSec","click", function(){
	...
	if (!msg){
		if ($("#dal_30 option:selected").val()==0) {
			msg = "Dal seçiniz.";
		}	
	}
	if (msg) {
		...
		var content = "<input type='hidden' id='bilgi_30' name='bilgi_30' value='"+$("#bolum_30 option:selected").val()+"-"+$("#seviye_30 option:selected").val()+"-"+$("#dal_30 option:selected").val()+"'/>";
		$("#bilgi_30").remove();
