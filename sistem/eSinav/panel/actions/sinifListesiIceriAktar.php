	...
	if ($_FILES || $_POST["bilgi_30"])
	{
			...
			foreach($rowIterator as $row){
				...
				foreach ($cellIterator as $cell) { //Veri Başlangıcını bul
					if (trim($cell->getCalculatedValue()) == "S.No") {
						$baslangicNoktasi = $row->getRowIndex ()+1;
						array_push($sutunlar,$cell->getColumn());
					}
					if (trim($cell->getCalculatedValue()) == "Giriş Yılı") {
						array_push($sutunlar,$cell->getColumn());
					}
					if (trim($cell->getCalculatedValue()) == "Öğrenci No") {
						array_push($sutunlar,$cell->getColumn());
					}
					if (trim($cell->getCalculatedValue()) == "Adı Soyadı") {
						array_push($sutunlar,$cell->getColumn());
					}
				}	
			}
			...
			foreach($rowIterator as $row){
				...
				foreach ($cellIterator as $cell) {
					if (trim($cell->getCalculatedValue())==": Devamından Muaf") {$array_data[$rowIndex]=null;$cikis=true;break;} // Liste Sonu
			...
			$r = mysql_fetch_object(mysql_query("SELECT * FROM ogretimturu WHERE id='".$bilgi[2]."'"));
			$ogretimTuru = $r->tur;
			...
			echo "<tr style='font-weight:bold'><td colspan='2' align='right'>Bölüm :</td><td colspan='2'>$bolumAdi</td></tr>";
			echo "<tr style='font-weight:bold'><td colspan='2' align='right'>Seviye :</td><td colspan='2'>$sinifSeviyesi</td></tr>";
			echo "<tr style='font-weight:bold'><td colspan='2' align='right'>Öğretim :</td><td colspan='2'>$ogretimTuru</td></tr>";
			echo "<tr style='font-weight:bold;background-color:#000;color:#FFF'><td>S.N.</td><td>Giriş Yılı</td><td>Okul No</td><td>Ad Soyad</td></tr>";
