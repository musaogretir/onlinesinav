...
require_once("headerActions.php");
	...
	if ($_POST){
		...
		$q1 = mysql_fetch_object(mysql_query("SELECT bolum,seviye,ogretimTuru FROM sinavlar WHERE id='$a'"));
		$q2 = "SELECT * FROM siniflisteleri WHERE ogretimYili = (SELECT id FROM ogretimyili WHERE aktif=1) AND donem = (SELECT id FROM donemler WHERE aktif = 1) AND bolum = '$q1->bolum' AND sinifSeviyesi = '$q1->seviye' AND ogretimTuru = '$q1->ogretimTuru'";
		...
		if (mysql_num_rows(mysql_query($q2))>0){ //Sınıf listesi girilmiş ise
			...
			if (!$q3){
				$q5 = mysql_fetch_object(mysql_query("SELECT bolum,seviye,ogretimTuru FROM sinavlar WHERE id='$a'"));
				$q6 = mysql_query("SELECT * FROM siniflisteleri WHERE ogretimYili = (SELECT id FROM ogretimyili WHERE aktif=1) AND donem = (SELECT id FROM donemler WHERE aktif = 1) AND bolum = '$q1->bolum' AND sinifSeviyesi = '$q5->seviye' AND ogretimTuru = '$q5->ogretimTuru'");
				...
				$hidden1 = implode(";",$insertArray);
				$hidden1 = base64_encode(base64_encode($hidden1));
				$hidden2 = base64_encode(base64_encode($a."-".$baslangic."-".$bitis));
				//Şifreleri Tablo Haline Getir
				$q7 = mysql_fetch_object(mysql_query("SELECT dersler.dersAdi,sinavlar.sinavTarihi,sinavlar.sinavSaati,sinavlar.seviye,sinavlar.ogretimTuru,kullanicilar.ad,kullanicilar.soyad FROM sinavlar INNER JOIN dersler ON dersler.id = sinavlar.ders INNER JOIN kullanicilar ON kullanicilar.id=sinavlar.ogretimGorevlisi WHERE sinavlar.id = '$a' "));
				$sn=1;
				...
				$msg.= "<tr height='30' style='background-color:#CFC'>
							...
							<td colspan='3'>$q7->dersAdi <font color='red'>[$q7->ogretimTuru .Öğretim - $q7->seviye .Sınıf]</font></td>
						<tr/>
