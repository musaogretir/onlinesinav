...
require_once("headerActions.php");
	...
	if ($_POST){
		...
		$q2 = mysql_query("SELECT * FROM siniflisteleri WHERE ogretimYili = (SELECT id FROM ogretimyili WHERE aktif=1) AND donem = (SELECT id FROM donemler WHERE aktif = 1) AND bolum = $q1->bolum AND sinifSeviyesi = $q1->seviye AND ogretimTuru = $q1->ogretimTuru");
