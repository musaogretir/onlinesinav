...
require_once("headerActions.php");
	...
	if ($_POST){
		...
		unset($_SESSION['notBilgisi']); //Bir önceki değerlendirme sonuç listesini sıfırla.
		$q1 = mysql_query("SELECT * FROM cevaplar WHERE sinav = '$id' ORDER BY ogrenci ASC");
		$s  = array();
		$t  = array();
		...
		$q = mysql_fetch_object(mysql_query("SELECT sinavlar.id, sinavlar.sinavTarihi, sinavlar.sinavSaati, sinifseviyeleri.seviye, dersler.dersAdi, ogretimturu.tur, kullanicilar.ad, kullanicilar.soyad FROM sinavlar INNER JOIN dersler ON dersler.id = sinavlar.ders INNER JOIN ogretimturu ON ogretimturu.id = sinavlar.ogretimTuru INNER JOIN sinifseviyeleri ON sinifseviyeleri.id = sinavlar.seviye INNER JOIN kullanicilar ON kullanicilar.id = sinavlar.ogretimGorevlisi WHERE ogretimGorevlisi = '".$_SESSION['gecerliKullanici']['userId']."' AND sinavDegerlendirmeDurumu = 0 AND donem = (SELECT id FROM donemler WHERE aktif = 1) AND ogretimyili = (SELECT id FROM ogretimyili WHERE aktif = 1) AND sinavlar.id='$id'"));
