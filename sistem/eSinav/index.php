...
<div id="cerceve">
	<form id="loginForm">
        ...
        <div class="satir">
        		<div class="soru">Öğretim Türü :</div>
                <div class="cevap">
                		<select name="ogTur" id="ogTur"  onchange="showValue(this.value)">
                      		<option value="0">Seçiniz</option>
                            <?php
								$bugun = date("d-m-Y");
								$q = mysql_query("SELECT ogretimturu.id,ogretimturu.tur FROM sinavlar INNER JOIN ogretimturu ON sinavlar.ogretimTuru = ogretimturu.id WHERE sinavlar.ogretimYili = (SELECT id FROM ogretimyili WHERE aktif=1) AND sinavlar.donem = (SELECT id FROM donemler WHERE aktif = 1) AND sinavlar.sifreOlusturmaDurumu = 1 AND sinavlar.sinavDegerlendirmeDurumu = 0 AND sinavlar.sinavTarihi = '$bugun'");
								$x = array();
								while($al = mysql_fetch_object($q)){
									if (!in_array($al->id,$x))
									{
										echo "<option value='$al->id'>$al->tur</option>";
										array_push($x,$al->id);
									}	
								}
							?>
        ...
        <div class="satir">
        		<div class="soru">Öğretim Görevlisi :</div>
                <div class="cevap">
                		<select name="ogGor" id="ogGor"  onchange="showValue(this.value)">
                                <option value="0">Seçiniz</option>
                                <?php
                                    $bugun = date("d-m-Y");
									$q = mysql_query("SELECT kullanicilar.id,kullanicilar.ad,kullanicilar.soyad FROM sinavlar INNER JOIN kullanicilar ON sinavlar.ogretimGorevlisi = kullanicilar.id WHERE sinavlar.ogretimYili = (SELECT id FROM ogretimyili WHERE aktif=1) AND sinavlar.donem = (SELECT id FROM donemler WHERE aktif = 1) AND sinavlar.sifreOlusturmaDurumu = 1 AND sinavlar.sinavDegerlendirmeDurumu = 0 AND sinavlar.sinavTarihi = '$bugun'");
									$x = array();
                                    while($al = mysql_fetch_object($q)){
										if (!in_array($al->id,$x))
										{
