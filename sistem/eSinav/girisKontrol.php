<?php
	...
	if ($_POST){
		...
		if ($c == $_SESSION['captcha']){
			...
			$q1 = mysql_fetch_object(mysql_query("SELECT id FROM sinavlar WHERE sinavTuru = '$sinTr' AND bolum = '$bolum' AND ders = '$ders' AND ogretimGorevlisi = '$ogGor' AND seviye = '$sinif' AND ogretimTuru = '$ogTur'  AND sinavDegerlendirmeDurumu = 0 AND ogretimYili = (SELECT id FROM ogretimyili WHERE aktif = 1) AND donem = (SELECT id FROM donemler WHERE aktif = 1)"));
			...
			if ($q1->id){
					...
					if ($q3){
							$q1 = mysql_fetch_object(mysql_query("SELECT * FROM sinavlar WHERE sinavTuru = '$sinTr' AND bolum = '$bolum' AND ders = '$ders' AND ogretimGorevlisi = '$ogGor' AND seviye = '$sinif' AND ogretimTuru = '$ogTur' AND sinavDegerlendirmeDurumu = 0  AND ogretimYili = (SELECT id FROM ogretimyili WHERE aktif = 1) AND donem = (SELECT id FROM donemler WHERE aktif = 1)"));
